defmodule Components.UserSession do
  ...
  defp via(user_id), do: {:via, Registry, {Components.UserSessionRegistry, user_id}}
  ...
  def start_supervised(initial_values) do
    ...
    case DynamicSupervisor.start_child(
           Components.UserSessionDynamicSupervisor,
  ...
  def count, do: Registry.count(Components.UserSessionRegistry)

  def lookup(user_id), do: Registry.lookup(Components.UserSessionRegistry, user_id)
  ...
  defp send_ws_impl(_platform, msg, state = %{pid: pid}) do
    ...
    if pid, do: Api.SocketHandler.remote_send(pid, msg)
    {:noreply, state}
  end
  ...
  defp set_active_ws(pid, _reply, state) do
    if state.pid do
      ...
      Adapters.Users.set_online(state.user_id)
    end
  ...
  defp handle_disconnect(pid, state = %{pid: pid}) do
    Adapters.Users.set_offline(state.user_id)
