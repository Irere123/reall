defmodule Api.Message.User.GetFollowers do
  ...
  defmodule Reply do
    ...
    @primary_key false
    embedded_schema do
      embeds_many(:followers, Schemas.User)
    ...
    with {:ok, %{username: username, cursor: cursor, limit: _limit}} <-
      ...
      {users, next_cursor} =
        if is_nil(username) do
          Components.Follow.get_follow_list(state.user.id, state.user.id, false, cursor)
        else
          Components.Follow.get_follow_list_by_username(state.user.id, username, false, cursor)
        end
